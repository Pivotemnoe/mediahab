        ...
        ideas.append(
            {
                ...
                "detail_questions": [
                    "Какой личный опыт или наблюдение ты можешь подтвердить?",
                    "Какая конкретная деталь важна, но пока не названа?",
                    "Какой честный вывод можно сделать из твоего опыта?",
                ],
            }
        )
        ...
        "ideas": [
            {
                ...
                "speaking_prompt": "Какой конкретный момент запомнился тебе?",
            },
            {
                "id": "idea-2",
                ...
                "speaking_prompt": "Какой реальный выбор заставил тебя задуматься?",
            },
            {
                "id": "idea-3",
                ...
                "speaking_prompt": "Какой этап процесса ты можешь описать по своему опыту?",
            },
            {
                "id": "idea-4",
                ...
                "speaking_prompt": "Что именно изменило твоё отношение к теме?",
            },
            {
                "id": "idea-5",
                ...
                "speaking_prompt": "Какой вопрос ты хочешь обсудить с читателями?",
            },
        ]
    }
...
def standalone_idea_system_prompt() -> str:
    return "\n".join(
        [
            ...
            "В speaking_prompt обращайся к автору только на «ты» и не используй формы «вы», «ваш», «вам» или «вас».",
...
def system_prompt(
    ...
    return "\n".join(
        [
            ...
            "Считай шуткой только законченный короткий образ, сравнение или олицетворение "
            "с понятной комической точкой. Абстрактные фразы вроде «показал характер», "
            "«в этом сценарии» или «всё неоднозначно» сами по себе шутками не считаются. "
            "Если правила просят три и более юмористических акцента, распредели их минимум "
            "по трём уместным частям публикации, а не складывай рядом. "
...
def cta_candidate(project_version: ProjectVersion) -> str:
    ...
    return "А ты как думаешь? Напиши в комментариях."
                ...
                StructuredGenerationRequest(
                    ...
                    system_prompt=(
                        ...
                        if attempt == 1
                            ...
                            "Создай новый полный JSON с теми же фактами, без двойных/парных "
                            "тире и без служебных комментариев о редактуре, ИИ или "
                            "проверке текста."
                        )
                    ),
                    user_prompt=prompt,
...
async def assemble_master(
    ...
    if not source_text_from_blocks(blocks).strip():
        raise AiPipelineError(
            ...
            "Сначала наговори или напиши основную мысль своими словами.",
        )
                ...
                StructuredGenerationRequest(
                    ...
                    system_prompt=(
                        ...
                        if attempt == 1
                            ...
                            "Верни полный новый вариант без двойных/парных тире и без служебных "
                            "комментариев о редактуре, ИИ или проверке текста."
                        )
                    ),
                    user_prompt=prompt,
                    fallback_payload={
