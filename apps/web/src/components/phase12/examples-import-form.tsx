...
export function ExamplesImportForm({
  ...
  const selectedCount = rows.length + (draft.trim() && !rows.some((row) => row.text === draft.trim()) ? 1 : 0);
      ...
      <div className="grid min-w-0 gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-muted">
            Вставляйте свои публикации или чужие тексты, чья подача вам близка. Используйте их как ориентир по ритму, структуре и тону; факты нового материала задаёт новая диктовка.
          </p>
        </div>
        <div className="min-w-44 rounded-xl border border-border bg-background p-3">
          <div className="flex items-end justify-between gap-3"><span className="text-xs text-muted">Готовность</span><strong className="text-lg text-primary">{readinessCount} из {RECOMMENDED_EXAMPLES}</strong></div>
          <progress aria-label={`Сохранено и выбрано до ${readinessCount} из ${RECOMMENDED_EXAMPLES} рекомендуемых постов`} className="mt-2 h-2 w-full accent-primary" max={RECOMMENDED_EXAMPLES} value={Math.min(readinessCount, RECOMMENDED_EXAMPLES)} />
          <p className="mt-1 text-xs leading-5 text-muted">Сохранено: {existingApprovedCount}. В этой загрузке: {selectedCount}.</p>
        </div>
      </div>
      ...
      <form className="grid min-w-0 gap-5" onSubmit={submit}>
        ...
        <div className="grid min-w-0 gap-3 rounded-xl border border-border bg-background p-4">
          <label className="grid min-w-0 gap-1.5 text-sm" htmlFor="style-example-draft">
            ...
            <textarea
              ...
              placeholder="Скопируйте публикацию целиком — вместе с заголовком, абзацами и финалом"
              value={draft}
            />
          </label>
          <div className="flex flex-wrap items-center gap-2">
            <Button onClick={addCurrentDraft} type="button" variant="secondary"><Plus size={16} />Добавить в подборку</Button>
        ...
        <details className="group rounded-xl border border-border bg-background">
          <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-4 text-sm font-semibold text-foreground outline-none focus-visible:ring-2 focus-visible:ring-ring/50">
            <span className="flex items-center gap-2"><FileStack size={17} />Вставить сразу несколько постов</span>
          ...
          <div className="grid gap-3 border-t border-border p-4">
            ...
            <Button className="justify-self-start" onClick={addBulkDraft} type="button" variant="secondary"><Plus size={16} />Разобрать и добавить</Button>
          </div>
        </details>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button disabled={isSubmitting || selectedCount === 0 || Boolean(disabledReason)} type="submit">
