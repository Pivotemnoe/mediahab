...
- `generation_runs`: workspace, проект, задача, провайдер, модель, ссылки на версии правил, расход, задержка, стоимость и ошибка. `rubric_id` и `content_item_id` могут быть пустыми для проектных задач, например генерации идей; tenant-доступ всегда определяется по `workspace_id`. Контекст, ID стилевых примеров, fingerprint prompt/schema/validator и попытки провайдера хранятся даже без материала, а принятая идея связывается с черновиком через первую ревизию и идемпотентное usage/событие активации.
