...
assert.match(viewSource, /«Наговори» подключает микрофон/);
...
assert.match(viewSource, /«Наговори» сохраняет запись/);
assert.match(viewSource, /«Наговори» отправляет запись и превращает её в текст/);
assert.match(viewSource, /Черновик сохранён/);
...
assert.match(viewSource, /Заметки отправляются/);
